...
`.env.example` の `GOOGLE_SERVICE_ACCOUNT_JSON` と `DRIVE_FOLDER_ID` を両方設定すると、「📁 ドキュメントライブラリ」内に「☁️ Googleドライブから読み込む」が表示され、指定フォルダ内のPDFを一覧から選んで読み込めるようになります（アップロード操作なしで直接利用可能）。設定手順は `.env.example` 内のコメントを参照してください。
