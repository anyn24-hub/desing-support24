...
文字情報を持たないPDF（スキャンした図面など）は、ライブラリ上で「スキャンPDF」と表示されます。この場合はテキスト抽出ではなく、**アプリ側でPDFの各ページをJPEG画像にレンダリングし、それをGeminiに直接読み取らせて**回答します（PDFファイルをそのまま渡す方式だと、大きい・複雑なスキャン文書でGemini側の処理上限に引っかかることがあるため、ページ画像に変換してから渡す方式にしています）。質問文に特定のファイル名を含めると、そのファイルのページ画像だけが送信されます（他の資料も含めたい場合はファイル名を指定しなければ全資料が対象になります）。
...
utils/
├── pdf_processor.py       # PDFからページ単位でテキスト抽出／スキャンPDFのページ画像レンダリング
