...
文字情報を持たないPDF（スキャンした図面など）は、ライブラリ上で「スキャンPDF」と表示されます。この場合、アップロード時に**アプリ側で無料のOCR（文字認識、Tesseract）を1回だけ実行してキャッシュ**し、文字として読み取れたページは通常のテキストPDFと同じ軽量な方式（テキストをそのままGeminiに渡す）で扱います。OCRで十分な文字が読み取れなかったページ（図面・写真中心のページなど）だけ、そのページをJPEG画像としてGeminiに直接読み取らせます。この仕組みにより、質問のたびに大量の画像データを送る必要がなくなり、無料枠のAPIリクエスト消費を大きく抑えられます。質問文に特定のファイル名を含めると、画像フォールバックが必要なページについてはそのファイルの分だけが送信されます（他の資料も含めたい場合はファイル名を指定しなければ全資料が対象になります）。

初回アップロード時はOCR処理のぶん時間がかかります（ページ数に比例）が、一度OCRされたページは同じ文書である限りキャッシュが使われ、再処理されません。ライブラリの一覧に各スキャンPDFのOCR結果（文字化できたページ数／画像のままのページ数）が表示されます。
